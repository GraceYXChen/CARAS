...
XParam_all = matrix(c(0,0, 1,0, 2,0, 4,0, 
                       0,0, 1,0, 2,0, 0,0, 
                       2,0, 2,1, 2,2,
                       4.5,-1, 1,1,  1,2,
                       0,0, 1,2),byrow = T,ncol=2)
...
XParam_all[12,] = c(5,-1)
IntParam_all = matrix(c(rep(0,14),-1,0,-1,0,
                        rep(0,10),rep(-1,4)),byrow=T,ncol=2)

XParam=c(2.6,-0.6)  #CARA1:0.17 CARA2:0.16
XParam=c(2.6,0)
XParam=c(4,0) # CARA1: 0.17
XParam = c(4, 0.9) # CARA1: 0.15
XParam=c(4, 0.8) # CARA1: 0.16
XParam=c(4, -0.8) # CARA1: 0.15
XParam=c(4, -0.9) # CARA1: 0.17
XParam=c(4.2, -0.9) # CARA1: 0.18
XParam=c(4.4, -0.9) # CARA1: 0.19
XParam=c(4.4, -1) # CARA1: 0.19
XParam=c(4.5,-0.9) # CARA1: 0.0.15

XParam=c(4, -1) # CARA1: 0.15
XParam = c(4, 0.7) # CARA1: 0.14
XParam = c(4,0.6) # CARA1: 0.14
...
for(iter in 1:1000){
  ...
  # print(c(mean(dif_ms,na.rm=T),mean(ms,na.rm=T),mean(dif01,na.rm=T),mean(H,na.rm=T)))
  # # print(mean(H,na.rm=T))
  # print(mean(rej,na.rm=T))
  ...
  Z.stat <- NULL; pv = NULL;
  Z.stat[1] <-  -(summary(fit1)$coefficients[1,4]); se1 <- summary(fit1)$coefficients[1,3]
  #Z.stat[1] <- (summary(fit1)$coefficients[1]-beta_H1)/summary(fit1)$coefficients[3]
  if(Z.stat[1] > a1){
    ...
    next}
      ...
      Z.stat[2] <-  -(summary(fit)$coefficients[1,4]);se2 <- summary(fit)$coefficients[1,3]      
      #Z.stat[2] <- (summary(fit)$coefficients[1]-beta_H1)/summary(fit)$coefficients[3]
      ...
      if(Z.stat[2] > a2){
        ...
        #dif_ms <- c(dif_ms, mean((data_2$del==1 & tt>=1)))
        #dif_ms <- c(dif_ms,RISCA:::rmst(trc_data2[trc_data2$G==1,]$y,trc_data2[trc_data2$G==1,]$del,0.5,"s"))
        #dif_ms <- c(dif_ms,dif_median_surv(data_2))
        #dif_ms <- c(dif_ms, mean(c(tt2)>3,na.rm=T))
        ...
        ms <- c(ms,dif_median_surv(data_2only))
        fail = c(fail, sum(trc_data2$del==1))
        #dif_ms <- c(dif_ms,RISCA:::rmst(trc_data2[trc_data2$G==1,]$y,trc_data2[trc_data3$G==2,]$del,0.5,"s"))
        #dif_ms <- c(dif_ms, mean((data_2$del==1 & tt>=1)))
        #dif_ms <- c(dif_ms,dif_median_surv(data_2))
        #dif_ms <- c(dif_ms, mean(c(tt2)>3,na.rm=T))
        dif_ms <- c(dif_ms,mean(data_2only$y<2 & data_2only$del==1,na.rm=T))
        next}else if((Z.stat[2]<a2)&(Z.stat[2]>b2)){
          ...
          #Z.stat[3] <- (summary(fit)$coefficients[1]-beta_H1)/summary(fit)$coefficients[3]
          Z.stat[3] <-  -(summary(fit)$coefficients[1,4])      
